...
class PromptTemplate:
    ...
    def __init__(self, templates_file_path: str):
        self.path = templates_file_path
        self.loader = YamlLoaderWithRandomization(templates_file_path)
        self.jinja_env = Environment(loader=self.loader)

    def render(self, key: str, **kargs):
        return  self.jinja_env.get_template(key).render(**kargs)


if "__main__" == __name__:
    t = PromptTemplate('template/example.yaml')
    speech_output = t.render('hi')
    assert speech_output == 'Hello World!'
    print(speech_output)
